...
export async function callGemini({
  ...
  for (let attempt = 1; attempt <= MAX_RETRIES; attempt++) {
    const activeModel = (attempt > 2 && modelName === "gemini-3.6-flash") ? "gemini-3.5-flash-lite" : modelName;
    try {
      ...
      const waitMs = is503 ? Math.pow(2, attempt) * 2000 : Math.pow(2, attempt) * 1000;
...
export async function safeCallGemini(options) {
  try {
    ...
    if (provider === "bedrock" || provider === "aws") {
      const { callBedrock } = await import("./bedrock.js");
      result = await callBedrock(options);
    } else {
